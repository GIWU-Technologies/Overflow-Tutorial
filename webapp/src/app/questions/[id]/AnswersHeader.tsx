import {ListBox, Select} from "@heroui/react";

type Props = {
    answerCount: number;
}

export default function AnswersHeader({answerCount}: Props) {
    return (
        <div className='flex items-center justify-between pt-3 w-full px-6'>
            <div className='text-2xl'>{answerCount} {answerCount === 1 ? 'Answer' : 'Answers'}</div>
            <div className='flex items-center gap-3 justify-end w-[50%] ml-auto'>
                <Select aria-label='Select Sorting' defaultSelectedKey={['highScore']}>
                    <Select.Trigger>
                        <Select.Value />
                        <Select.Indicator />
                    </Select.Trigger>
                    <Select.Popover>
                        <ListBox>
                            <ListBox.Item key='highScore' id='highScore'>Highest Score (default)</ListBox.Item>
                            <ListBox.Item key='created' id='created'>Date created</ListBox.Item>
                        </ListBox>
                    </Select.Popover>
                       
                    
                </Select>
            </div>
        </div>
    );
}